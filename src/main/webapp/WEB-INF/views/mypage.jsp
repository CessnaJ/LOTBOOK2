    ...
    <div class="container">
        <div class="row checkout__order">
            ...
            <button type="submit" class="site-btn"
                    onclick="location.href='/member/myinfo'">회원 정보 수정
            </button>
        </div>
    </div>
</section>
        ...
        <div class="checkout__form">
            ...
            <c:choose>
                ...
                <c:otherwise>
                                                        ...
                                                        <p class="font-italic text-danger"
                                                           id="point${product.sequence }">
                                                            <c:set var="totalPoint"
                                                                   value="${((product.price * product.count) * product.pointAccumulationRate * 0.01) - ((product.price * product.count) * product.pointAccumulationRate * 0.01)%1 }"/>
                    ...
                    <button type="submit" class="site-btn bg-danger text-white border-0 rounded-sm col-12"
                            onclick="cart_to_order()">주문하기
                    </button>
                </c:otherwise>
            </c:choose>
                        ...
                        <div class="card mb-3 border-0">
                            ...
                            <c:forEach items="${order.orderDetailList}" var="orderDetail">
                                        ...
                                        <div class="d-flex flex-row align-items-center">
                                            ...
                                            <div style="width: 100px;">
                                                <h5 class="mb-0" style="font-size: 15px; font-weight: 700;"
                                                    id="price${product.sequence }">
                                                    <c:set var="price"
                                                           value="${orderDetail.productPrice * orderDetail.count}"/>
                                                    <fmt:formatNumber type="number" maxFractionDigits="3"
                                                                      value="${price}"/>
                                                    원</h5>
                                            </div>
                                ...
                                <div id="reviewCollapse-${order.sequence}-${orderDetail.sequence}"
                                     class="collapse">
                                    <form name="frm" id="ratingForm" action="/api/review/register">
                                        <input
                                                type="hidden" name="memberSequence"
                                                value="${logincust.sequence }"> <input type="hidden"
                                                                                       name="productSequence"
                                                                                       value="${orderDetail.productSequence}">
                                        <input type="hidden"
                                               name="orderdetailSequence" value="${orderDetail.sequence}">
                                        <input class="rating-input" type="hidden"
                                               id="star-input-${orderDetail.sequence}" name="rating" required value="">
                                        ...
                                        <div class="alert alert-danger mt-4"
                                             ...
                                             style="display: none;">별점을 클릭해주세요
                                        </div>
                                        <div class="alert alert-danger mt-4"
                                             id="alert-2"
                                             style="display: none;">리뷰를 20자 이상 작성해주세요
                                        </div>
                    ...
                    <c:forEach items="${myReviewList}" var="review" varStatus="status">
                        <div class="card mb-3 border-0">
                            ...
                            <div id="reviewList-${review.sequence}">
                                <form id="review-edit-${review.sequence }" action="/review/update" method="post"
                                            ...
                                            <c:forEach var="i" begin="1" end="5">
                                                <c:choose>
                                                    <c:when test="${i <= review.rating}">
                                                        <i class="fa fa-star active star-update-${review.sequence}"
                                                           id="star-update-${review.sequence}"
                                                           style="pointer-events:none;"></i>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <i class="fa fa-star star-update-${review.sequence}"
                                                           id="star-update-${review.sequence}"
                                                           style="pointer-events:none;"></i>
                                                    </c:otherwise>
                                                </c:choose>
                                ...
                                <form id="review-delete-${review.sequence }" action="/review/delete" method="post">
                                    ...
                                    <i class="fa fa-trash-o" aria-hidden="true"
                                       style="position:absolute; right: 5%; top:10%; cursor:pointer; font-size: 130%;"
                                       onclick="confirmDelete(${review.sequence });"></i>
                                </form>
                            </div>
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary text-light" data-dismiss="modal"
                        onclick="close_modal()">아니오
                </button>
                <button id="delete_yes" class="btn btn-danger text-light"
                        data-dismiss="modal" onclick="cart_delete()">&nbsp;&nbsp;&nbsp;&nbsp;예&nbsp;&nbsp;&nbsp;&nbsp;
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    function cart_delete() {
        location.href = '/cart/deleteCart?sequence=' + productSequence + '&memberSequence=' + memberSequence + '&isCart=0';
